...
export function LiveETA({
  ...
  const view = useMemo(() => {
    ...
    const fee = shippingFeeIdr(zone, weightKg);
    return { dispatch, win, fee };
  }, [zone, cutoffLocal, weightKg, tick]);
  ...
  return (
    <div className="rounded-xl border border-brand-line bg-black/40 p-4">
      ...
      <div className="mt-3 flex items-center justify-between border-t border-brand-line pt-3 text-xs text-brand-muted">
        ...
        <span className="font-semibold text-brand-text">{formatPrice(view.fee, currency)}</span>
      </div>
    </div>
  );
}
