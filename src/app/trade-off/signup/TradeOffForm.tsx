          ...
          <div className="flex items-center gap-4">
            <div className="h-24 w-24 shrink-0 overflow-hidden rounded-full border border-brand-line bg-brand-surface">
              {state.avatar_url ? (
                ...
                <div className="flex h-full w-full items-center justify-center text-2xl font-bold text-brand-accent">
                  {(state.display_name.trim().charAt(0) || "?").toUpperCase()}
                </div>
              )}
            </div>
            <div className="flex flex-col gap-2">
              <label className="inline-flex h-11 cursor-pointer items-center justify-center rounded-lg border border-brand-line bg-brand-surface px-4 text-xs font-semibold text-brand-text transition hover:border-brand-accent hover:text-brand-accent">
                <input
                  ref={avatarInputRef}
                  type="file"
                  accept="image/*"
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {state.photos.map((url, i) => (
            ...
            >
              ...
              {i === 0 && (
                <span className="absolute left-2 top-2 rounded-full bg-brand-accent px-2 py-0.5 text-[11px] font-bold text-black">
                  Cover
                </span>
              )}
          ...
          {state.photos.length < 6 && (
            <label className="flex aspect-square cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-brand-line bg-brand-surface p-4 text-center text-xs text-brand-muted transition hover:border-brand-accent hover:text-brand-accent">
              <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="space-y-4 rounded-2xl border border-brand-line bg-brand-surface/40 p-5">
      <h2 className="text-xs font-bold uppercase tracking-widest text-brand-accent">{title}</h2>
