...
const BANNER = "https://ik.imagekit.io/9mrgsv2rp/ChatGPT%20Image%20Jun%2020,%202026,%2006_09_59%20AM.png";
...
// First gallery media row (sort_order asc, kind=image)
...
if (m.data) {
  ...
  console.log("✓ hero gallery row updated");
} else {
  const ins = await s.from("hammerex_product_media").insert({
    product_id: id, kind: "image", url: BANNER,
